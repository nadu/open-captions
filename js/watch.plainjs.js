...
function init(){
	...
	function showASL(word,i){
		...
		word = word.replace(/[\.\!\?\,]/g,"");
		getASLPage(word);
		document.getElementById("imgWrapper").innerHTML = "<div id='aslWrapper' style='background-color:white; opacity=1; width: 260px; height:300px; right:10px; position:fixed; background-position:center;background-repeat:no-repeat; border-radius: 6px;-moz-border-radius: 6px; z-index:1000; border: 2px black solid;'><div style='width:260px; height:300px; text-align:center'><iframe id='iframe-0' style='margin-left:-10px;' class=' iframe-delta-0' height='180' width='220' frameborder='0' scrolling='no' name='iframe-0' src='http://cats.gatech.edu/cats/MySignLink/dictionary/html/pages/"+word.toLowerCase()+".htm'/></div></div>";
		...
		var loadTimer = setTimeout(function(){
						...
						if((document.getElementById("aslWrapper")))
